  ...
  class AdminBookingController {
    ...
    static async assignServiceProvider(req, res, next) {
      ...
      try {
        ...
        for (const item of booking.BookingItems) {
          if (item.serviceItem) {
            ...
            if (service?.Service?.ServiceType?.SubCategory?.ServiceCategory) {
              categoryIds.add(service.Service.ServiceType.SubCategory.ServiceCategory.category_id);
            }
            ...
            if (packageItem?.PackageSection?.Package?.ServiceType?.SubCategory?.ServiceCategory) {
              categoryIds.add(packageItem.PackageSection.Package.ServiceType.SubCategory.ServiceCategory.category_id);
            }
          }
        }
        ...
        if (!provider) {
          throw createError(404, 'Service provider not found or does not serve this city/category');
        }
        ...
        if (booking.service_location && provider.primary_location) {
          ...
          if (distance > cityServiceInfo.service_radius) {
            throw createError(400, 'Service location is outside provider\'s service radius for this city');
          }
        }
    ...
    static async updateBookingStatus(req, res, next) {
      ...
      try {
        ...
        if (!validStatusTransitions[booking.status]?.includes(status)) {
          throw createError(400, `Invalid status transition from ${booking.status} to ${status}`);
        }
  
        const updates = { 
          status,
          customer_notes: notes ? `${booking.customer_notes || ''}\n${notes}` : booking.customer_notes
        };
        ...
        if (status === 'cancelled') {
          ...
          if (booking.BookingPayment && booking.BookingPayment.payment_status === 'completed') {
